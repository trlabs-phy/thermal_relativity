...
# Description:
# ------------
# This module implements the causal-to-spatial translation layer
# for the Unified Thermal Relativity Boltzmann Solver.
#
# It constructs static three-dimensional spatial snapshots from
# a certified one-dimensional causal history. These snapshots
# represent pure geometric embeddings of an already-evolved
# causal state.
#
...
# - no causal logic,
# - no thresholds,
# - no feedback into the solver.
#
# All operations performed here are non-causal and strictly
# downstream of the history integrator and certification layers.
#
# Architectural guarantees:
# --------------------------
# - Reads recorded causal history only
# - Performs pure spatial embedding and bookkeeping
# - Does not modify solver state or certification outcomes
# - Preserves η as the authoritative causal coordinate
#
# Governing role:
# ---------------
# - Constructs static 3D bubble snapshots for certification
# - Defines spatial masks, domains, and bookkeeping fields
# - Supports strong-field, projection, and structural diagnostics
# - Provides geometry-only inputs to downstream analysis
#
# This module is a translation layer, not a solver component.
#
# ============================================================
...
def build_bubble_snapshot(
    ...
    *,
    ...
    V_univ_series = history.get("V_univ", [])
    if idx >= len(V_univ_series):
        raise IndexError("Snapshot idx out of range")

    V_univ = float(V_univ_series[idx])
    ...
    # ------------------------------
    # Metadata (causal-neutral)
    # ------------------------------
    meta = {
        "idx": int(idx),
        "eta": eta,
        "snapshot_reason": snap_reason,
        "snapshot_role": "UNSPECIFIED",
        "V_univ": V_univ,
    ...
    return BubbleSnapshot(
        ...
        escape_weight=np.ones_like(E_cell),
